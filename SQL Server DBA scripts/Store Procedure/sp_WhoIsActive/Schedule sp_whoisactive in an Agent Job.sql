...
_/*************************************************************************
Code Description: How to Schedule sp_whoisactive in an Agent Job
Author			: Brent Ozar
Link			: https://www.brentozar.com/responder/log-sp_whoisactive-to-a-table/

Create a new job and plop the below code into the job step, modifying the first 3 variables as needed. 
The code will create the logging table if it doesn’t exist, the clustered index if it doesn’t exist, log current activity and purge older data based on the @retention variable.

How often should you collect activity? 
I think collecting sp_WhoIsActive data every 30-60 seconds is a good balance between logging enough activity to troubleshoot production problems and the storage needed to keep the data in a very busy environment.
...
If you run this, and then upgrade sp_WhoIsActive to a newer version that outputs more columns, the inserts will fail because the newer columns don’t exist. 
The easiest way to fix that is drop the output table whenever you upgrade sp_WhoIsActive, and this code will just regenerate the table with the latest results structure. 
If you need to preserve your old sp_WhoIsActive data, you’ll need to alter the output table manually to include any new columns that have been added in the newer version of sp_WhoIsActive.
